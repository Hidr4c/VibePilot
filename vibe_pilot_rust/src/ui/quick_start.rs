...
pub fn render_help_tab(ui: &mut egui::Ui, app: &mut VibePilotApp) {
    ...
    // Help Text / Instructions (At the top)
    ui.group(|ui| {
        ...
        ui.label(egui::RichText::new(help_title).strong());
        ui.separator();
        ui.label(help_step1);
        ui.label(help_step2);
        ui.label(help_step3);
